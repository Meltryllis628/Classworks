...
class Banker:
    ...
    def request_resources(self, customer_index, request):
        ...
        ### BEGIN ANSWER HERE ###
        availableCopy = deepcopy(self.available)
        needCopy = deepcopy(self.need)
        allocationCopy = deepcopy(self.allocation)
        safe = self.check_safe(customer_index, request, availableCopy, needCopy, allocationCopy)
    ...
    def check_safe(self, customer_index, request, work, need, allocation):
        ...
        #       2. need[customer_index][i] = need[customer_index][i] - request[i] for all i<M
            need[customer_index][j] = need[customer_index][j] - request[j]
        #       3. allocation[customer_index][i] = allocation[customer_index][i] + request[i] for all i<M
            allocation[customer_index][j] = allocation[customer_index][j] + request[j]
